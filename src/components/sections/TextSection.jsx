...
export default function TextSection({ data, mt = true, mb = true }) {
  return (
    <div className={`${mt ? typeof mt === 'string' ? mt : 'mt-8' : ''} ${mb ? typeof mb === 'string' ? mb : 'mb-8' : ''}`}>
...
const components = {
  block: {
    h2: props => (
      <h2 className={`text-4xl font-bold ${defaultMargins}`}>{props?.children}</h2>
    ),
    h3: props => (
    ...
    blockquote: props => (
      <blockquote className={`italic before:content-['“'] after:content-['”'] ${defaultMargins}`}>{props?.children}</blockquote>
    ),
    normal: props =>{
      return (
      <p className={`${defaultMargins}`}>{props?.children}</p>
    )},
  },
  marks: {
